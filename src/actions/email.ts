...
export async function sendContact(prevState: any, formData: FormData) {
  ...
  if (!resendKey) {
    return { error: "Service d'email non configuré. Contactez l'administrateur." };
  }
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: process.env.EMAIL_FROM ?? 'Prime Rénov <site@primerenov.fr>',
      to: [process.env.EMAIL_TO ?? 'contact@primerenov.fr'],
